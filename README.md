...
生信分析路线：
- 1.当我们数据下机之后，得到的fastq文件，使用`fastp`软件对IgG（内参）和IP（目标蛋白）样本的原始数据，进行质控（这一步主要是去除接头、去除重复序列以及低质量序列）。得到clean data。（该软件能否去除重复序列存疑，我之前用的是`Trimmomatic`只有去接头和去除低质量序列的功能。）
